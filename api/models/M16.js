module.exports = {
  async getData(watchlist) {
    var conditionString = '(';
    watchlist.forEach((ticker) => {
      conditionString += `'${ticker}', `;
    });
    ...
    var sql = `SELECT mt1.*,mt2.cv,mt2.cv5,mt2.roc1,mt2.timex,ABS(mt1.tsl-mt2.c) AS gap, mt2.C,mt2.TOTALSIG 
              FROM 
                ...
                FROM trend_score15ms WHERE id IN (SELECT MAX(id) FROM trend_score15ms GROUP BY A10) AND DATE LIKE (SELECT MAX(DATE) FROM trend_score15ms) AND  ticker IN ${conditionString}) mt1
              LEFT JOIN 
